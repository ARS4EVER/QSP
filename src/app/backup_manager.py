...
负责将文件分割为 Shamir 秘密份额，并分发给网络中的参与者
支持断点续传、本地加密存储和远程份额推送
"""
...
class BackupManager:
    ...
    将文件分割为 n 个 Shamir 秘密份额，分布式存储在多个节点上
    支持本地份额保存和远程份额推送
    """
    
    BLOCK_SIZE = 1024 * 1024  # 1MB 分块大小
    FRAGMENT_SIZE = 1024      # 片段大小
