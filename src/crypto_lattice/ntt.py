...
class NTT:
    ...
    def ntt(self, poly):
        ...
        """
        # [安全修正] 确保输入转换为普通 int 列表，防止 NumPy 溢出
        a = [int(x) for x in poly] 
        n = self.n
        q = self.q
...
def polymul_rq(poly_a, poly_b):
    ...
    # [关键修复] 预先将输入转换为 Python 原生 int 列表
    # 这避免了当输入为 numpy array (如 int32) 时，乘法溢出导致的计算错误
    # 错误现象：overflow encountered in scalar multiply
    if hasattr(poly_a, 'tolist'):
        list_a = poly_a.tolist()
    else:
        list_a = list(poly_a)
        
    if hasattr(poly_b, 'tolist'):
        list_b = poly_b.tolist()
    else:
        list_b = list(poly_b)
        
    # 强制类型转换为 int，确保使用 Python 的大整数运算
    list_a = [int(x) for x in list_a]
    list_b = [int(x) for x in list_b]
    
    for i in range(n):
        coeff_a = list_a[i] # 预取优化
        # 只有当 coeff_a 不为 0 时才进行内层循环 (稀疏优化)
        if coeff_a == 0:
            continue
            
        for j in range(n):
            # 乘法：使用原生 int 进行计算，避免 NumPy 溢出
            val = (coeff_a * list_b[j]) % q
