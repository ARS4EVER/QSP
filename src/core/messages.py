...
使用 MessagePack 替代 JSON/Base64，消除 33% 数据膨胀
"""
...
class RecoveryMsgType(Enum):
    """恢复消息类型枚举"""
    REQ_RECOVERY = 0x01  # 请求恢复
    RESP_SHARE = 0x02    # 响应份额
    ERROR = 0xFF          # 错误消息
...
class RecoveryMessage:
    ...
    支持二进制（MessagePack + zlib压缩）和 JSON + Base64 两种格式
    """
    
    USE_BINARY = True  # 默认使用二进制格式以减少数据膨胀
